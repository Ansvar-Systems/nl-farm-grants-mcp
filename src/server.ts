...
const SERVER_NAME = 'nl-farm-grants-mcp';
...
const TOOLS = [
  ...
  {
    name: 'search_grants',
    description: 'Search Dutch farm grants by keyword. Covers ISDE, SDE++, JOLA, MIT, WBSO, Sbv, Stoppersregeling, Borgstellingsfonds, and more.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        query: { type: 'string', description: 'Free-text search query (e.g. "warmtepomp", "stikstof", "jonge landbouwer")' },
        grant_type: { type: 'string', description: 'Filter by grant type (e.g. capital, revenue, tax_credit, buyout)' },
        min_value: { type: 'number', description: 'Minimum grant value in EUR' },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        grant_id: { type: 'string', description: 'Grant ID (e.g. isde, sde-plus-plus, jola, stoppersregeling)' },
        jurisdiction: { type: 'string', description: 'ISO 3166-1 alpha-2 code (default: NL)' },
      },
      required: ['grant_id'],
    },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        grant_id: { type: 'string', description: 'Grant ID (e.g. isde, sde-plus-plus, jola)' },
        category: { type: 'string', description: 'Filter by item category (e.g. warmtepomp, isolatie, zonneboiler, innovatie)' },
        jurisdiction: { type: 'string', description: 'ISO 3166-1 alpha-2 code (default: NL)' },
      },
      required: ['grant_id'],
    },
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        grant_id: { type: 'string', description: 'Grant ID (e.g. isde, jola, stoppersregeling)' },
        jurisdiction: { type: 'string', description: 'ISO 3166-1 alpha-2 code (default: NL)' },
      },
      required: ['grant_id'],
    },
  },
  {
    ...
    inputSchema: {
      type: 'object' as const,
      properties: {
        grant_id: { type: 'string', description: 'Grant ID (e.g. isde, jola, borgstellingsfonds)' },
        items: {
          type: 'array',
          items: { type: 'string' },
          description: 'Array of item codes to include. If omitted, includes all items.',
        },
        area_ha: { type: 'number', description: 'Area in hectares (for per-hectare payment items)' },
        jurisdiction: { type: 'string', description: 'ISO 3166-1 alpha-2 code (default: NL)' },
      },
      required: ['grant_id'],
    },
